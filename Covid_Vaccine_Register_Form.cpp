...
typedef struct mynode //create alias for another DT
{
	char name[20];
	char gen[6];
	char idtype[40];
	...
	char mob[20];
	char comor[3];
	struct mynode* link; //pointer to an element of a structure node     //data and address section that holds the address of the next element in the list
} Node; //represent the struct
Node* start = NULL; //empty linked list in beginning
...
void details()
{
	...
	for (i = 1; i <= n; i++)
		...
		fflush(stdin); //purpose to clear
		gets(a); //read the string
...
// Create Node & insert the data
void addnode(char a[20], char b[6], char c[40], char d[20],char e[20], char f[3])
{
	Node *newptr = NULL, *ptr;
	newptr = (Node*)malloc(sizeof(Node)); //allocate single memory(return NULL) //size of a variable
	strcpy(newptr->name, a); // malloc:(ptr=(data type*) malloc(byte size);
	strcpy(newptr->gen, b); //allow the element for structure & union(pointer variable pointing the structure)
...
void venue()
{
	...
	// For Clear Screen(clear the console.... lib fun located in the console ip)
	system("cls");
	...
	printf("\t\t\t\t1. DORA Hospital\n");
	printf("\t\t\t\t2. SHINCHAN Hospital\n");
	printf("\t\t\t\t3. GOVERNMENT Hospital\n");
	...
	do {
		...
		scanf("%d", &i);
		if (i == 1)
			strcpy(hos, "DORA Hospital");
		else if (i == 2)
			strcpy(hos, "SHINCHAN Hospital");
		else if (i == 3)
			strcpy(hos, "GOVERNMENT Hospital");
		else {
